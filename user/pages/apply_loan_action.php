...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    function uploadFile($fileKey) {
        ...
        // Check file type (you can extend this check based on your requirements)
        $allowedFileTypes = ['image/jpeg', 'image/png', 'application/pdf'];
        if (!in_array($file['type'], $allowedFileTypes)) {
            $errors[] = "Error: Invalid file type for '$fileKey'.";
            return null;
        }
    ...
    if ($designation === 'business') {
        $sql = "INSERT INTO `apply_data` (`Id`, `fullName`, `email`, `phone`, `whatsapp_phone`, `loan_Type`, `loan_Amount`, `pancard`, `aadharcard`, `bankstat`, `passphoto`, `Desigination`, `income_tax_return`, `salary_slip`)
                VALUES 
                ('$id', '$fullName', '$email', '$phone', '$whatsAppPhone', '$loanType', '$loanAmount', '$pancard', '$aadharcard', '$bankStatement', '$passportPhoto', '$designation', '$incomeTaxReturn', NULL)";
    } elseif ($designation === 'salary') {
        $incomeTaxReturn = '';
        $sql = "INSERT INTO `apply_data` (`Id`, `fullName`, `email`, `phone`, `whatsapp_phone`, `loan_Type`, `loan_Amount`, `pancard`, `aadharcard`, `bankstat`, `passphoto`, `Desigination`, `income_tax_return`, `salary_slip`)
                VALUES 
                ('$id', '$fullName', '$email', '$phone', '$whatsAppPhone', '$loanType', '$loanAmount', '$pancard', '$aadharcard', '$bankStatement', '$passportPhoto', '$designation', NULL, '$salarySlip')";
    } else {
